...
valid_set=dev-clean
# test_sets="test-clean_fake test-clean"
# test_sets="test-clean_fake-xvector"
# test_sets="test-clean test-clean_fake"
# test_sets="test-clean_fake test-clean"
test_sets="test-clean"

train_config=conf/train.yaml
inference_config=conf/decode.yaml
# inference_config=conf/tuning/decode_fastspeech_perturb_spk.yaml
...
./tts.sh \
    ...
    --use_xvector true \
    --use_tgt_xvector true \
    --use_second_tgt_xvector true \
    --token_type phn \
    --cleaner "${cleaner}" \
    --g2p "${g2p}" \
    --train_config "${train_config}" \
    --inference_config "${inference_config}" \
    --inference_tag "three_stage_random_tgt_spembs2" \
    --train_set "${train_set}" \
    --valid_set "${valid_set}" \
    --test_sets "${test_sets}" \
    --download_model "kan-bayashi/libritts_tts_train_xvector_conformer_fastspeech2_transformer_teacher_raw_phn_tacotron_g2p_en_no_space_train.loss" \
